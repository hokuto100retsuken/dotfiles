...
2. レビューコメントの取得
   ...
   - 解決状態を厳密に見たい場合は GraphQL の `reviewThreads { isResolved comments }` を使う（REST の comments レスポンスには `isResolved` が無い）
   - `gh pr view {number} --comments` でPR全体のコメントも確認
3. 対応が必要なコメントの整理
   - 未解決の指摘のみリストアップする
     - インラインコメント: reviewThreads の `isResolved=false`、それが取れない場合は返信がないものを未解決とみなす
...
5. 修正の実施
   - 指摘内容に基づいてコードを修正する
   - 修正ごとにコミットする（コミットメッセージは日本語で、指摘内容に対応した簡潔な文にする）
6. 結果の報告
   - 何を修正したかを簡潔に報告する
